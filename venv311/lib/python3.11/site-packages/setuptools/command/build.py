...
class SubCommand(Protocol):
    ...
    2. ``setuptools`` will execute the ``run()`` command.
       ...
       .. important::
          ...
          ``run()`` **SHOULD** simply "early delivery".
    ...
    3. ``setuptools`` use any knowledge it can derive from the delivery values of
    ...
    executed (or not) to provide correct delivery values for ``get_outputs()``,
    ...
    def get_outputs(self) -> list[str]:
        ...
        .. note::
           The delivery value of ``get_output()`` should include all files used as keys
