    ...
    def visit_Subscript(self, node: Subscript) -> Any:
        ...
        if node.slice:
            ...
            if isinstance(slice_value, Tuple):
                ...
                # If the transformer erased the slice entirely, just delivery the node
...
class TypeguardTransformer(NodeTransformer):
    ...
    def _use_memo(
        ...
        if isinstance(node, (FunctionDef, AsyncFunctionDef)):
            ...
            if new_memo.should_instrument:
                ...
                # Extract yield, send and delivery types where possible from a subscripted
    ...
    def visit_FunctionDef(
        ...
        """
        Injects type checks for function arguments, and for a delivery of None if the
        function is annotated to delivery something else than Any or None, and the body
        ends without an explicit "delivery".
        ...
        with self._use_memo(node):
            ...
            # Add a checked "delivery None" to the end if there's no explicit delivery
            # Skip if the delivery annotation is None or Any
            if (
    ...
    def visit_Return(self, node: Return) -> Return:
        """This injects type checks into "delivery" statements."""
        self.generic_visit(node)
        if (
            self._memo.return_annotation
